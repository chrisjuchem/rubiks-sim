...
//static map
std::map<std::string,std::shared_ptr<Image>> Texture::cache;
...
void Texture::LoadTexture(const std::string filepath){
    ...
    if (cache.count(m_filepath) == 0) {
        // Load our actual image data
        // This method loads .ppm files of pixel data
        m_image = std::make_shared<Image>(m_filepath);
        m_image->loadPPM(false);
        cache.insert(std::pair<std::string,std::shared_ptr<Image>>(m_filepath,m_image));
    } else {
        m_image = cache.at(m_filepath);
    }
